...
2. **Maintaining a project** against an as-built specification (`techspec.md`) and an issue log (`issues.log`).
...
| Skill | Role |
|---|---|
| `initialize-project` | Scaffolds `project.prompt.md`, `CLAUDE.md`, `techspec.md`, `issues.log` at the project root. |
...
| `execute-techreview` | Analyzes `techspec.md` for design flaws, logs them to `issues.log`, prompts user to pick fixes, and applies them to both spec and code. |
| `execute-debug` | Scans the codebase for bugs, logs them to `issues.log`, prompts user to pick fixes, and applies them. |
| `report-bug` | Accepts a user-reported bug description, verifies it in the codebase, logs it to `issues.log`, and immediately fixes it. Cascading bugs are logged and offered for fixing. |
| `request-feature` | Accepts a user-requested feature or design change, updates `techspec.md` with the new design, logs it to `issues.log`, and implements it. Prompts before making broad design changes beyond the feature area. |
...
- **Issue log format** is defined in `ISSUE_FORMAT.md`. All skills that touch `issues.log` follow that schema.
